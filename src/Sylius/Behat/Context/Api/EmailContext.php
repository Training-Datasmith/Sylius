...
final readonly class Email_Context implements Context
{
    ...
    {
        $this->assert_email_contains_message_to($this->translator->trans('sylius.email.user.account_verification.message'), $recipient);
    }
    ...
    {
        $this->assert_email_contains_message_to(sprintf('%s %s %s', $this->translator->trans('sylius.email.order_confirmation.your_order_number', [], null, $locale_code), $order->get_number() ?? $this->shared_storage->get('order_number'), $this->translator->trans('sylius.email.order_confirmation.has_been_successfully_placed', [], null, $locale_code)), $recipient);
    }
