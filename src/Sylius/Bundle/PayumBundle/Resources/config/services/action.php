...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius_payum.action.authorize_payment', AuthorizePaymentAction::class)
        ->args([service('sylius_payum.provider.payment_description')])
        ->tag('payum.action', ['all' => true, 'alias' => 'sylius.authorize_payment'])
    ;

    $services
        ->set('sylius_payum.action.capture_payment', CapturePaymentAction::class)
        ->args([service('sylius_payum.provider.payment_description')])
        ->tag('payum.action', ['all' => true, 'alias' => 'sylius.capture_payment'])
    ;

    $services
        ->set('sylius_payum.action.execute_same_request_with_payment_details', ExecuteSameRequestWithPaymentDetailsAction::class)
        ->tag('payum.action', ['all' => true])
    ;

    $services
        ->set('sylius_payum.action.resolve_next_route', ResolveNextRouteAction::class)
        ->tag('payum.action', ['all' => true, 'alias' => 'sylius.resolve_next_route'])
    ;

    $services
        ->set('sylius_payum.action.offline.convert_payment', ConvertPaymentAction::class)
        ->tag('payum.action', ['factory' => 'offline', 'alias' => 'sylius.offline.convert_payment'])
    ;

    $services
        ->set('sylius_payum.action.offline.status', StatusAction::class)
        ->tag('payum.action', ['factory' => 'offline', 'alias' => 'sylius.offline.status'])
    ;

    $services
        ->set('sylius_payum.action.offline.resolve_next_route', ResolveNextOfflineRouteAction::class)
