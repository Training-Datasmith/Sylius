...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius.doctrine.orm.event_subscriber.load_metadata.attribute', LoadMetadataSubscriber::class)
    ...
    $services
        ->set('sylius.registry.attribute_type', ServiceRegistry::class)
        ->args([
    ...
    $services
        ->set('sylius.form.type.attribute_type_choice', AttributeTypeChoiceType::class)
        ->args(['%sylius.attribute.attribute_types%'])
        ->tag('form.type')
    ;

    $services
        ->set('sylius.validator.attribute_type', AttributeTypeValidator::class)
        ->args([service('sylius.registry.attribute_type')])
        ->tag('validator.constraint_validator', ['alias' => 'sylius_attribute_type_validator'])
    ;

    $services
        ->set('sylius.validator.valid_attribute_value', ValidAttributeValueValidator::class)
        ->args([service('sylius.registry.attribute_type')])
        ->tag('validator.constraint_validator', ['alias' => 'sylius_valid_attribute_value_validator'])
    ;

    $services
        ->set('sylius.validator.valid_text_attribute_configuration', ValidTextAttributeConfigurationValidator::class)
