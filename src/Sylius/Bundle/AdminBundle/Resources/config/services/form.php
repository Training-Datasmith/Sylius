...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius_admin.form.type.request_password_reset', RequestPasswordResetType::class)
        ->args(['%sylius.form.type.admin.password_reset_request.validation_groups%'])
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.reset_password', ResetPasswordType::class)
        ->args(['%sylius.form.type.admin.reset_password.validation_groups%'])
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.grid_filter.ux_autocomplete', UxAutocompleteFilterType::class)
        ->tag('form.type')
        ->tag('ux.entity_autocomplete_field')
    ;

    $services
        ->set('sylius_admin.form.type.grid_filter.ux_translatable_autocomplete', UxTranslatableAutocompleteFilterType::class)
        ->tag('form.type')
        ->tag('ux.entity_autocomplete_field')
    ;

    $services
        ->set('sylius_admin.form.extension.type.promotion', PromotionTypeExtension::class)
        ->args([
    ...
    $services
        ->set('sylius_admin.form.extension.type.promotion_action.product_filter_configuration', ProductFilterConfigurationTypeExtension::class)
        ->args([service('sylius.form.type.data_transformer.products_to_codes')])
        ->tag('form.type_extension')
    ;

    $services
        ->set('sylius_admin.form.extension.type.promotion_action.taxon_filter_configuration', TaxonFilterConfigurationTypeExtension::class)
    ...
    $services
        ->set('sylius_admin.form.extension.type.promotion_rule.contains_product_configuration', ContainsProductConfigurationTypeExtension::class)
        ->args([service('sylius.repository.product')])
        ->tag('form.type_extension')
    ;

    $services
        ->set('sylius_admin.form.extension.type.promotion_rule.has_taxon_configuration', HasTaxonConfigurationTypeExtension::class)
        ->args([service('sylius.form.type.data_transformer.taxons_to_codes')])
        ->tag('form.type_extension')
    ;

    $services
        ->set('sylius_admin.form.extension.type.promotion_rule.total_of_items_from_taxon_configuration', TotalOfItemsFromTaxonConfigurationTypeExtension::class)
        ->args([service('sylius.repository.taxon')])
        ->tag('form.type_extension')
    ;

    $services->set('sylius_admin.form.type.admin_user', AdminUserType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.address', AddressType::class)
        ->args([service('sylius.repository.country')])
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.catalog_promotion', CatalogPromotionType::class)
        ->args([
    ...
    $services
        ->set('sylius_admin.form.type.catalog_promotion_action', CatalogPromotionActionType::class)
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.catalog_promotion_scope', CatalogPromotionScopeType::class)
        ->args([tagged_iterator('sylius_admin.catalog_promotion.scope_configuration_type', indexAttribute: 'key')])
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.catalog_promotion_scope.for_products_scope_configuration', ForProductsScopeConfigurationType::class)
        ->args([service('sylius.form.type.data_transformer.products_to_codes')])
    ...
    $services
        ->set('sylius_admin.form.type.catalog_promotion_scope.for_taxons_scope_configuration', ForTaxonsScopeConfigurationType::class)
        ->args([service('sylius.form.type.data_transformer.taxons_to_codes')])
    ...
    $services
        ->set('sylius_admin.form.type.catalog_promotion_scope.for_variants_scope_configuration', ForVariantsScopeConfigurationType::class)
    ...
    $services->set('sylius_admin.form.type.channel', ChannelType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.channel_price_history_config', ChannelPriceHistoryConfigType::class)
        ->args([
    ...
    $services
        ->set('sylius_admin.form.type.country', CountryType::class)
        ->args([service('sylius.repository.country')])
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.currency', CurrencyType::class)
        ->args([service('sylius.repository.currency')])
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.customer', CustomerType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.customer_group', CustomerGroupType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.exchange_rate', ExchangeRateType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.locale', LocaleType::class)
        ->args([service('sylius.repository.locale')])
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.order', OrderType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.payment_method', PaymentMethodType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.product', ProductType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.product_association_type', ProductAssociationTypeType::class)
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.product_attribute', ProductAttributeType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.product_attribute_autocomplete', ProductAttributeAutocompleteType::class)
    ...
    $services
        ->set('sylius_admin.form.type.product_autocomplete', ProductAutocompleteType::class)
    ...
    $services
        ->set('sylius_admin.form.type.product_generate_variants', ProductGenerateVariantsType::class)
        ->tag('form.type')
    ;

    $services
        ->set('sylius_admin.form.type.product_image', ProductImageType::class)
        ->args(['%sylius.model.product_variant.class%'])
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.product_option', ProductOptionType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.product_review', ProductReviewType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.product_variant', ProductVariantType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.product_variant_autocomplete', ProductVariantAutocompleteType::class)
        ->args(['%sylius.model.product_variant.class%'])
        ->tag('form.type')
        ->tag('ux.entity_autocomplete_field')
    ;

    $services->set('sylius_admin.form.type.promotion', PromotionType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.promotion_coupon', PromotionCouponType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.promotion_coupon_generator_instruction', PromotionCouponGeneratorInstructionType::class)
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.shipping_category', ShippingCategoryType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.shipping_method', ShippingMethodType::class)
        ->args(['%sylius.shipping_method_rules%'])
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.shipping_method_rule', ShippingMethodRuleType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.shipment_ship', ShipmentShipType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.shop_user', ShopUserType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.tax_category', TaxCategoryType::class)->tag('form.type');

    $services->set('sylius_admin.form.type.taxon', TaxonType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.taxon_autocomplete', TaxonAutocompleteType::class)
        ->args(['%sylius.model.taxon.class%'])
        ->tag('form.type')
        ->tag('ux.entity_autocomplete_field')
    ;

    $services->set('sylius_admin.form.type.tax_rate', TaxRateType::class)->tag('form.type');

    $services
        ->set('sylius_admin.form.type.translatable_autocomplete', TranslatableAutocompleteType::class)
        ->args([service('sylius.context.locale')])
        ->tag('form.type')
    ;

    $services->set('sylius_admin.form.type.zone', ZoneType::class)->tag('form.type');
};
