...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius_admin.provider.pending_action.count_orders_to_process', CountOrdersToProcessProvider::class)
        ->args([service('sylius.repository.order')])
    ;

    $services
        ->set('sylius_admin.provider.pending_action.count_pending_payments', CountPendingPaymentsProvider::class)
        ->args([service('sylius.repository.payment')])
    ;

    $services
        ->set('sylius_admin.provider.pending_action.count_product_reviews_to_approve', CountProductReviewsToApproveProvider::class)
        ->args([service('sylius.repository.product_review')])
    ;

    $services
        ->set('sylius_admin.provider.pending_action.count_product_variants_out_of_stock', CountProductVariantsOutOfStockProvider::class)
        ->args([service('sylius.repository.product_variant')])
    ;

    $services
        ->set('sylius_admin.provider.pending_action.count_shipments_to_ship', CountShipmentsToShipProvider::class)
