...
final class TranslatableAutocompleteType extends AbstractType
{
    ...
    {
        $resolver->setDefault('choice_label', fn (Options $options, mixed $previousValue): mixed => $options['extra_options']['choice_label'] ?? $previousValue);

        $resolver->setDefault('choice_value', fn (Options $options, mixed $previousValue): mixed => $options['extra_options']['choice_value'] ?? $previousValue);

        $resolver->setDefault('entity_fields', fn (Options $options): array => $options['extra_options']['entity_fields'] ?? ['code']);

        $resolver->setAllowedTypes('entity_fields', ['array', 'null']);
        $resolver->setNormalizer('entity_fields', fn (Options $options, ?array $entityFields): array => array_map(
            fn (string $field): string => self::ENTITY_ALIAS . '.' . $field,
            $entityFields ?? [],
        ));

        $resolver->setDefault('translation_fields', fn (Options $options): array => $options['extra_options']['translation_fields'] ?? ['name']);
        $resolver->setAllowedTypes('translation_fields', 'array');
        $resolver->setNormalizer('translation_fields', fn (Options $options, array $translationFields): array => array_map(
            fn (string $field): string => self::TRANSLATION_ALIAS . '.' . $field,
            $translationFields,
        ));
        ...
        $resolver->setNormalizer(
            'filter_query',
            fn (Options $options, ?callable $filterQuery): callable => $this->normalizeFilterQuery($options, $filterQuery),
        );
    }
