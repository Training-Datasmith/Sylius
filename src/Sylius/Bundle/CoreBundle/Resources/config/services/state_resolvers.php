...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius.state_resolver.order', OrderStateResolver::class)
        ->args([service('sylius_abstraction.state_machine')])
    ;

    $services
        ->set('sylius.state_resolver.checkout', CheckoutStateResolver::class)
        ->args([
    ...
    $services
        ->set('sylius.state_resolver.order_payment', OrderPaymentStateResolver::class)
        ->args([service('sylius_abstraction.state_machine')])
    ;

    $services
        ->set('sylius.state_resolver.order_shipping', OrderShippingStateResolver::class)
        ->args([service('sylius_abstraction.state_machine')])
    ;
};
