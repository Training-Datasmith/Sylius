...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius.listener.workflow.order.assign_order_number', AssignOrderNumberListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.assign_order_token', AssignOrderTokenListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.create_payment', CreatePaymentListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.create_shipment', CreateShipmentListener::class)
        ->args([service('sylius_abstraction.state_machine')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.create', 'priority' => 500])
    ;

    $services
        ->set('sylius.listener.workflow.order.decrement_promotion_usages', DecrementPromotionUsagesListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.increment_promotion_usages', IncrementPromotionUsagesListener::class)
        ->args([service('sylius.modifier.promotion.order_usage')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.create', 'priority' => 300])
    ;

    $services
        ->set('sylius.listener.workflow.order.hold_inventory', HoldInventoryListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.give_back_inventory', GiveBackInventoryListener::class)
        ->args([service('sylius.operator.inventory.order_inventory')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.cancel', 'priority' => 200])
    ;

    $services
        ->set('sylius.listener.workflow.order.request_order_payment', RequestOrderPaymentListener::class)
        ->args([service('sylius_abstraction.state_machine')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.create', 'priority' => 700])
    ;

    $services
        ->set('sylius.listener.workflow.order.request_order_shipping', RequestOrderShippingListener::class)
        ->args([service('sylius_abstraction.state_machine')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.create', 'priority' => 800])
    ;

    $services
        ->set('sylius.listener.workflow.order.save_customer_addresses', SaveCustomerAddressesListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.set_immutable_names', SetImmutableNamesListener::class)
    ...
    $services
        ->set('sylius.listener.workflow.order.cancel_order_payment', CancelOrderPaymentListener::class)
        ->args([service('sylius_abstraction.state_machine')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.cancel', 'priority' => 400])
    ;

    $services
        ->set('sylius.listener.workflow.order.cancel_order_shipping', CancelOrderShippingListener::class)
        ->args([service('sylius_abstraction.state_machine')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.cancel', 'priority' => 300])
    ;

    $services
        ->set('sylius.listener.workflow.order.cancel_payment', CancelPaymentListener::class)
        ->args([service('sylius_abstraction.state_machine')])
        ->tag('kernel.event_listener', ['event' => 'workflow.sylius_order.completed.cancel', 'priority' => 600])
    ;

    $services
        ->set('sylius.listener.workflow.order.cancel_shipment', CancelShipmentListener::class)
        ->args([service('sylius_abstraction.state_machine')])
