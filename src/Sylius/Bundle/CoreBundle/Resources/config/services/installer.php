...
return static function (ContainerConfigurator $container): void {
    ...
    $services
        ->set('sylius.checker.installer.command_directory', CommandDirectoryChecker::class)
        ->args([service('filesystem')])
    ;

    $services
        ->set('sylius.checker.installer.sylius_requirements', SyliusRequirementsChecker::class)
        ->args([service('sylius.requirements.installer.sylius')])
    ;

    $services
        ->set('sylius.provider.installer.database_setup_commands', DatabaseSetupCommandsProvider::class)
        ->args([service('doctrine.orm.entity_manager')])
    ;
    $services->alias(DatabaseSetupCommandsProviderInterface::class, 'sylius.provider.installer.database_setup_commands');

    $services
        ->set('sylius.setup.installer.currency', CurrencySetup::class)
        ->args([
    ...
    $services
        ->set('sylius.setup.installer.locale', LocaleSetup::class)
        ->args([
    ...
    $services
        ->set('sylius.setup.installer.channel', ChannelSetup::class)
        ->args([
