...
final class CompositeShippingMethodEligibilityCheckerPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->has('sylius.checker.shipping_method_eligibility')) {
            return;
        }

        $container->getDefinition('sylius.checker.shipping_method_eligibility')->setArguments([
            array_map(
                static fn (int|string $id): Reference => new Reference($id),
